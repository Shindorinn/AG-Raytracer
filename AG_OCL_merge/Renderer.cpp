...
Tmpl8::Renderer::Renderer(Scene* scene, Surface* renderSurface)
{
	this->scene = scene;
...
vec3 Tmpl8::Renderer::Trace(Tmpl8::Ray* ray, int x, int y)
{
	...
	if (smallestT == INFINITY) {
		...
		if (hit->material.materialKind == Material::MaterialKind::DIFFUSE)
		{
			...
			{
				vec3 direction = vec3::normalize(scene->lights[i]->position - intersectionPoint);
...
vec3 Tmpl8::Renderer::DirectIllumination(vec3 intersectionPoint, vec3 direction, vec3 normal, Light* lightSource, Material material)
{
	vec3 intersectionWithEpsilon = intersectionPoint + EPSILON * direction;
	Tmpl8::Ray shadowRay = Tmpl8::Ray(intersectionWithEpsilon, direction);
	...
	float euclidianDistanceToLight = vec3EuclidianDistance(intersectionPoint, lightSource->position);
