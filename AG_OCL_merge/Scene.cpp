...
Tmpl8::Scene::Scene()
{
	...
	lights[0] = new Tmpl8::Light(vec3(-3, 2, -3), vec3(70, 70, 70));
	lights[1] = new Tmpl8::Light(vec3(3, -3, -5), vec3(70, 70, 70));
	...
	Tmpl8::Triangle* tri1 = new Tmpl8::Triangle(vec3(8, -8, 8), vec3(-8, -8, 8), vec3(-8, 8, 8));
	tri1->material = Material(vec3(1, 1, 1), Material::MaterialKind::DIFFUSE);
	primitives[counter++] = tri1;

	Tmpl8::Triangle* tri2 = new Tmpl8::Triangle(vec3(8, -8, 8), vec3(-8, 8, 8), vec3(8, 8, 8));
...
Tmpl8::AABB* Tmpl8::Scene::CalculateSceneBounds()
{
	...
	for (int i = 0; i < sizeof(this->primitives) / sizeof(this->primitives[0]); i++) {
		Tmpl8::AABB* currentAABB = primitives[i]->boundingBox;
