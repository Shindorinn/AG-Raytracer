...
bool Tmpl8::Kernel::clinitialized = false, Tmpl8::Kernel::candoInterop = true;
char* Tmpl8::Kernel::log = 0;
cl_context Tmpl8::Kernel::context;
cl_command_queue Tmpl8::Kernel::queue;
cl_device_id Tmpl8::Kernel::device;
...
#endif
	std::printf( "available OpenCL platforms:\n" );
...
// Write
// ----------------------------------------------------------------------------
void Tmpl8::Buffer::Write(void* dst)
{
	unsigned char* data = (unsigned char*)clEnqueueMapBuffer(Tmpl8::Kernel::GetQueue(), pinnedBuffer, CL_TRUE, CL_MAP_WRITE, 0, 4 * size, 0, NULL, NULL, NULL);
	memcpy( data, dst, size * 4 );
	clEnqueueWriteBuffer(Tmpl8::Kernel::GetQueue(), deviceBuffer, CL_FALSE, 0, 4 * size, data, 0, NULL, NULL);
}

// Read
// ----------------------------------------------------------------------------
void Tmpl8::Buffer::Read(void* dst)
{
	unsigned char* data = (unsigned char*)clEnqueueMapBuffer(Tmpl8::Kernel::GetQueue(), pinnedBuffer, CL_TRUE, CL_MAP_READ, 0, 4 * size, 0, NULL, NULL, NULL);
	clEnqueueReadBuffer(Tmpl8::Kernel::GetQueue(), deviceBuffer, CL_TRUE, 0, 4 * size, data, 0, NULL, NULL);
	memcpy( dst, data, size * 4 );
}

// Buffer constructor
// ----------------------------------------------------------------------------
Tmpl8::Buffer::Buffer(unsigned int N, unsigned int t, void* ptr)
{
	type = t;
	ownData = false;
	if (t == DEFAULT)
	{
		...
		deviceBuffer = clCreateBuffer(Tmpl8::Kernel::GetContext(), CL_MEM_READ_WRITE, size * 4, 0, 0);
		if (!ptr) 
		{
	...
	else
	{
		...
		if (Tmpl8::Kernel::candoInterop)
		{
		#ifdef USE_CPU_DEVICE
			deviceBuffer = clCreateFromGLTexture2D( Kernel::GetContext(), CL_MEM_READ_WRITE, GL_TEXTURE_2D, 0, N, 0 );
		#else
			if (t == TARGET) deviceBuffer = clCreateFromGLTexture(Tmpl8::Kernel::GetContext(), CL_MEM_WRITE_ONLY, GL_TEXTURE_2D, 0, N, 0);
			else deviceBuffer = clCreateFromGLTexture(Tmpl8::Kernel::GetContext(), CL_MEM_READ_ONLY, GL_TEXTURE_2D, 0, N, 0);
		#endif
			hostBuffer = 0;
		}
		else
		{
			...
			if (t == TARGET) deviceBuffer = clCreateImage(Tmpl8::Kernel::GetContext(), CL_MEM_WRITE_ONLY, &format, &desc, hostBuffer, 0);
			else deviceBuffer = clCreateImage(Tmpl8::Kernel::GetContext(), CL_MEM_READ_WRITE, &format, &desc, hostBuffer, 0);
		}
	}
}
...
// CopyToDevice method
// ----------------------------------------------------------------------------
void Tmpl8::Buffer::CopyToDevice(bool blocking)
{
	cl_int error;
...
// ----------------------------------------------------------------------------
void Tmpl8::Buffer::CopyFromDevice(bool blocking)
{
	cl_int error;
...
// CopyTo
// ----------------------------------------------------------------------------
void Tmpl8::Buffer::CopyTo(Buffer* buffer)
{
...
// Kernel constructor
// ----------------------------------------------------------------------------
Tmpl8::Kernel::Kernel(char* file, char* entryPoint)
{
	if (!clinitialized)
	{
...
// SetArgument methods
// ----------------------------------------------------------------------------
void Tmpl8::Kernel::SetArgument( int idx, cl_mem* buffer ) { clSetKernelArg( kernel, idx, sizeof( cl_mem ), buffer ); }
void Tmpl8::Kernel::SetArgument( int idx, Tmpl8::Buffer* buffer) { clSetKernelArg(kernel, idx, sizeof(cl_mem), buffer->GetDevicePtr()); }
void Tmpl8::Kernel::SetArgument( int idx, int value ) { clSetKernelArg( kernel, idx, sizeof( int ), &value ); }
void Tmpl8::Kernel::SetArgument( int idx, float value ) { clSetKernelArg( kernel, idx, sizeof( float ), &value ); }
void Tmpl8::Kernel::SetArgument( int idx, vec2 value ) { clSetKernelArg( kernel, idx, sizeof( vec2 ), &value ); }
void Tmpl8::Kernel::SetArgument( int idx, vec3 value ) { clSetKernelArg( kernel, idx, sizeof( vec3 ), &value ); }
void Tmpl8::Kernel::SetArgument( int idx, vec4 value ) { clSetKernelArg( kernel, idx, sizeof( vec4 ), &value ); }

// Run method
// ----------------------------------------------------------------------------
void Tmpl8::Kernel::Run()
{
	glFinish();
...
// Run method
// ----------------------------------------------------------------------------
void Tmpl8::Kernel::Run(cl_mem* buffers, int count)
{
	glFinish();
	cl_int error;
...
void Tmpl8::Kernel::Run(Tmpl8::Buffer* buffer)
{
	glFinish();
	cl_int error;
	if (Kernel::candoInterop)
...
void Tmpl8::Kernel::Run(const size_t count)
{
	cl_int error;
