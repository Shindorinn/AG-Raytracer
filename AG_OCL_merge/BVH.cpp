...
void Tmpl8::BVH::Traverse(Tmpl8::Ray* ray, BVHNode* node, bool isShadowRay)
{
...
float Tmpl8::BVH::IntersectPrimitives(Tmpl8::Ray* ray, Tmpl8::BVHNode* node)
{
...
void Tmpl8::BVH::ConstructBVH(Tmpl8::Primitive** primitives)
{
...
float Tmpl8::BVH::PointBoundsDist(Tmpl8::AABB bounds, vec3 point)
{
...
Tmpl8::AABB Tmpl8::BVH::CalculateBounds(Tmpl8::Primitive** primitives, int first, int last)
{
	float minx = INFINITY, miny = INFINITY, minz = INFINITY, maxx = -INFINITY, maxy = -INFINITY, maxz = -INFINITY;

	for (int i = first; i < last; i++)
	{
		Tmpl8::AABB* bounds = primitives[i]->boundingBox;
	...
	return Tmpl8::AABB(vec3(minx, miny, minz), vec3(maxx, maxy, maxz));
}