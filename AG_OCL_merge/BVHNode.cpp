...
void Tmpl8::BVHNode::Subdivide(Tmpl8::BVHNode** pool, Primitive** primitives, uint& poolPtr)
{
#if !USESAH
...
bool Tmpl8::BVHNode::Partition(Tmpl8::BVHNode** pool, Tmpl8::Primitive** primitives, uint& poolPtr)
{
#if USESAH
	...
	for (int i = 1; i < 7; i++)
	{
		for (int dimension = 0; dimension < 3; dimension++)
		{
			...
			float splitCost = Tmpl8::AABB(leftMin, leftMax).GetVolume() * leftCounter + Tmpl8::AABB(rightMin, rightMax).GetVolume() * rightCounter;
...
void Tmpl8::BVHNode::UpdateBounds(Tmpl8::AABB* bounds, vec3& min, vec3& max)
{
