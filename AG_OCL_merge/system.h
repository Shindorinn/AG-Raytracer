...
#define USE_GLEE		// instead of glew; potentially faster but not as safe
...
#include "lib\SDL2-2.0.3\include\SDL_syswm.h"
//#include "wglext.h"
...
public:
	...
	vec3 operator * (const vec3& operand) const { return vec3(x * operand.x, y * operand.y, z * operand.z); }
	vec3 operator / (float divOperand) { return vec3(x / divOperand, y / divOperand, z / divOperand); }
	...
	float vec3EuclidianDistance(const vec3 operand) const { return (vec3(x,y,z) - operand).length(); }
};

float vec3EuclidianDistance(const vec3& a, const vec3& b);
...
namespace Tmpl8 {
	...
	public:
		...
		vec4 operator - (const vec4& operand) const { return vec4(x - operand.x, y - operand.y, z - operand.z, w - operand.w); }
		vec4 operator * (const vec4& operand) const { return vec4(x * operand.x, y * operand.y, z * operand.z, w * operand.w); }
		vec4 operator * (float operand) const { return vec4(x * operand, y * operand, z * operand, w * operand); }
		void operator -= (const vec4& a) { x -= a.x; y -= a.y; z -= a.z; w -= a.w; }
		void operator += (const vec4& a) { x += a.x; y += a.y; z += a.z; w += a.w; }
		void operator *= (const vec4& a) { x *= a.x; y *= a.y; z *= a.z; w *= a.w; }
		void operator *= (float a) { x *= a; y *= a; z *= a; w *= a; }
