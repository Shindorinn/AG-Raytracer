...
void Game::KeyDown(int a_Key)
{
	...
	if (a_Key == 79 || a_Key == 80)
	{
		if (a_Key == 80)
			...
			camera->moveYawPitch(-1 * rotationSpeed, 0.0f);
		else
			camera->moveYawPitch(rotationSpeed, 0.0f);
	}
	//UpDown
	if (a_Key == 82 || a_Key == 81)
	{
		if (a_Key == 82)
			camera->moveYawPitch(0.0f, -1 * rotationSpeed);
		else
			camera->moveYawPitch(0.0f, rotationSpeed);
	}
	//Left/RightCtrl
