...
Camera::Camera()
{
	...
	p0 = screenCenter + vec4(-1.0f, -1.0f, 0.0f, 0.0f);
	p1 = screenCenter + vec4( 1.0f, -1.0f, 0.0f, 0.0f);
	p2 = screenCenter + vec4(-1.0f,  1.0f, 0.0f, 0.0f);
}
...
void Camera::GenerateRays()
{
	...
	for (int y = 0; y < SCRHEIGHT; y++){
		for (int x = 0; x < SCRWIDTH; x++)
		{
			...
			vec4 direction = normalize(pointOnScreen - position);

			Ray* ray = new Ray(position.xyz, direction.xyz);
