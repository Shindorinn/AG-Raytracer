...
public:
    ...
    Ray makeRay(int x, int y) const
    {
        ...
        return Ray(mOrigin, glm::normalize(lookVec), STARTING_TTL);
    };
    ...
    {
        mDirty = true;
        mFov = clamp(mFov + d, EPSILON, PI-EPSILON);
    }
    ...
    void moveYawPitch(float deltaYaw, float deltaPitch)
    {
        ...
        mYaw = std::fmod(mYaw + deltaYaw, PI*2);
